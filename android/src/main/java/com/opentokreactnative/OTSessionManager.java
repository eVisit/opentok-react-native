...
public class OTSessionManager extends ReactContextBaseJavaModule
    ...
    @Override
    public void onAudioStats(PublisherKit publisher, PublisherKit.PublisherAudioStats[] publisherAudioStats) {
        String publisherId = Utils.getPublisherId(publisher);
        if (publisherId.length() == 0) {
            return;
        }
        String event = publisherId + ":" + publisherPreface + "onAudioStats";
        for (int i = 0; i < publisherAudioStats.length; i++) {
            WritableMap publisherInfo = Arguments.createMap();
            publisherInfo.putMap("stream", EventUtils.prepareJSStreamMap(publisher.getStream(), publisher.getSession()));
            publisherInfo.putMap("audioStats", EventUtils.preparePublisherAudioNetworkStats(publisherAudioStats[i]));
            sendEventMap(this.getReactApplicationContext(), event, publisherInfo);
        }
    }

    @Override
    public void onVideoStats(PublisherKit publisher, PublisherKit.PublisherVideoStats[] publisherVideoStats) {
        String publisherId = Utils.getPublisherId(publisher);
        if (publisherId.length() == 0) {
            return;
        }
        String event = publisherId + ":" + publisherPreface + "onVideoStats";
        for (int i = 0; i < publisherVideoStats.length; i++) {
            WritableMap publisherInfo = Arguments.createMap();
            publisherInfo.putMap("stream", EventUtils.prepareJSStreamMap(publisher.getStream(), publisher.getSession()));
            publisherInfo.putMap("videoStats", EventUtils.preparePublisherVideoNetworkStats(publisherVideoStats[i]));
            sendEventMap(this.getReactApplicationContext(), event, publisherInfo);
        }
    }
}
